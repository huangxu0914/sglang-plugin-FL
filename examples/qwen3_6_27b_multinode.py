...
Supported TP sizes: 1, 2, 3, 4, 6, 8, 12, 24 (num_attention_heads=24)
  ...
  For 2 nodes × 8 GPUs, use TP=8 with CUDA_VISIBLE_DEVICES to select 4 GPUs
  per node, or use TP=6/12/24 if hardware permits.
============================================================================
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Qwen3.6-27B multi-node verification")
    parser.add_argument(
        "--role",
...
class TestRunner:
    ...
    def summary(self, tp_size: int, nnodes: int):
        ...
        print(f"  Model:   Qwen3.6-27B  TP: {tp_size}  Nodes: {nnodes}")
        print("=" * 56)
        if self.failed == 0:
...
def run_master(args):
    ...
    # Validate TP size for this model
    num_heads = 24
    if num_heads % args.tp != 0:
        print(
            f"ERROR: TP={args.tp} is not supported for Qwen3.6-27B (num_attention_heads={num_heads})"
        )
        print(
            f"Supported TP sizes: {[d for d in range(1, num_heads + 1) if num_heads % d == 0]}"
        )
        sys.exit(1)
