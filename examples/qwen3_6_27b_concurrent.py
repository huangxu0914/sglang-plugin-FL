...
"""Qwen3.6-27B (Dense) concurrent inference test with sglang-plugin-FL.
...
Usage:
  python qwen3_6_27b_concurrent.py [--mode text|vl|mixed|all]
...
def run_mixed_concurrent(engine):
    ...
    async def run():
        t0 = time.perf_counter()
        text_coros = [text_one(p) for _, p in text_items]
        vl_coros = [vl_one(p, u) for _, p, u in vl_items]
        all_results = await asyncio.gather(*text_coros, *vl_coros)
        return time.perf_counter() - t0, all_results[:n_text], all_results[n_text:]

    elapsed, text_results, vl_results = engine.loop.run_until_complete(run())

    all_latencies = [lat for lat, _ in text_results] + [lat for lat, _ in vl_results]
    total_tokens = sum(
        r.get("meta_info", {}).get("completion_tokens", 0)
        for _, r in list(text_results) + list(vl_results)
    )
    _report("mixed-concurrent", CONCURRENT_N, elapsed, all_latencies, total_tokens)

    t_pairs = [
        (label, r["text"]) for (label, _), (_, r) in zip(text_items, text_results)
    ]
    v_pairs = [(case, r["text"]) for (case, _, _), (_, r) in zip(vl_items, vl_results)]
    return t_pairs, v_pairs
